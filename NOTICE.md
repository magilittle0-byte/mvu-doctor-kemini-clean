...
- `legacy/0.7.5/`：重建前旧 Doctor 的只读历史快照。根目录 0.9.0 运行时不会导入或调用它。
...
第三方文件保留其原作者信息和原许可证状态；具体来源、哈希、复用等级和不得不新写的原因见 `docs/0.8.0-REFERENCE-TRANSPLANT-SOURCE-MAP.md` 至 `docs/0.9.0-NATIVE-WORLD-OWNERSHIP-SOURCE-MAP.md`。
