...
- `vendor/world-engine-v3.0.2/`：World Engine v3.0.2 的完整上游运行基线，固定提交 `154de4b590378cd0bd851cfffcefd3d96741cf3f`。44 个文件保持原字节；`world-engine-evolution.js` 只在原生 `extraInstructions` 任务数组末尾增加一个可选 Doctor provider 调用。`UPSTREAM_SHA256SUMS.txt` 固定上游 45 文件原哈希，`SOURCE_SHA256SUMS.txt` 固定当前 44+1 运行哈希；`SOURCE_CMP_REPORT.txt` 明确只保存适配前的历史导入比对。
- `vendor/life-state-v5.35/`：用户提供的角色生理状态引擎 ver5.35 脚本 JSON、双正则 JSON及逐字节提取的脚本内容；`PROVENANCE.json` 和验证脚本记录三份 SHA-256。
- `legacy/0.7.5/`：重建前旧 Doctor 的只读历史快照。根目录 0.9.3 运行时不会导入或调用它；0.9.1 仅按来源图移植其中已验证的变量证据分栏语义，0.9.2 仅复用其五标签网格与移动端无横溢布局原则；0.9.3 的世界修复来自冻结的 World 3.0.2 原件，不回接旧 Doctor 世界架构。
- `index.js`、`profile-engine.js`、`style.css`：仅承担 Story变量复检、精确楼层/swipe身份、人物档案原子持久化、World 对话机制块清理、同楼 accepted-final 屏障、首次世界书选择、正文 `buildContext` 与 World→Memory 联动的可观察投影、统一报告与控制台。两处投影都只改交给原函数的副本，不删改完整持久 World 或原生推演输入；Memory 原生 `replace` 回滚和空摘要跳过语义保持不变。World 3.0.2 的原生 scheduler、`evolve`、存档、重roll、设置和UI不由Doctor接管；有真实助手正文时只等待本楼变量与人物阶段进入终态，阶段失败仍释放原生 World，手动推进等无助手正文路径不等待。
