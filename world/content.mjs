...
export function worldInstruction(input, baseline = {}) {
  ...
  return [
    ...
    '未公开的企图、真实身份、私人解释、离场过程保存在 blackbox；公开 events 的 name/desc、winds 的 topic/content/source 和 regionalIncident 的可见影响，只写确已到达当前场景的外部迹象及后果。沿用原生字段及字数上限，在 desc/content/impact 中简明交代具体物件或行为、到达的地点或传播入口、正在生效的变化；有时间或接触前提时写清该前提。用客观环境变化、NPC行为、声音、痕迹或有来源的消息形成正文可采用的入口，不只写幕后原因或抽象阶段。可观察不等于玩家已经看见、听见、理解或作出反应。远方发生的事若无传播渠道继续保密，不把完整内幕投射到玩家视角。',
