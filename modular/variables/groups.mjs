...
对每项先在Analysis中引用本卡check里决定字段所属范围、创建或更新时机的原句，再逐个判断这些条件是否成立，给出对应的用户明确输入或已接受正文依据。这个判断先于寻找同名记录或补齐空值：正文、面板或数据库将某事物称为同一类别，不等于它属于该完整路径，也不能替代原check的前提。条件成立后再比较当前值与本轮结束应有值；条件未成立而已有错写时纠正错写，不能沿用错误去补全整项。区分明确创建登记、已发生变化和仍未完成的行动；用户明确创建设定仍按本卡创建规则登记，不要求正文重复。必要差额才给补丁；无差额也说明为什么不需要维护。
