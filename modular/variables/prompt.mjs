...
保护已确认的身份、天赋和设定，先区分固定角色设定与待玩家填写的创建模板。用户明确提交当前角色的创建资料或更正时，按本卡创建规则登记对应字段；不能把前态或模板默认值当成已经确认的身份，也不要求正文再复述一次才算设定成立。普通提及、署名或猜测不自动成为当前角色赋值；没有明确创建或更正依据时，不改写既有身份。规则中的条件句必须保留条件，创建资料登记不等于已经执行后续行动，示例、规划、选项、NPC尝试和未裁决结果不能充当已完成事实。
