# MVU 模块医生（0.10.27 单次诊断候选）

当前先重新验证变量模块：每次自动检查或点击“修复本轮”最多请求模型一次，完整读取正文、历史、规则和 MVU；失败不自动追加请求。原有字段保护、官方解析与保存读回仍保留。106 项受控检查通过，**尚未完成真实门禁，当前未锁定**。见[调用预算与来源](docs/modular/PHASE1_CALL_BUDGET.md)。人物与世界模块等待 P1 重新验收。

以下为历史阶段记录，不能作为 0.10.27 的验收依据；旧 P1 锁及证据已原样归档。
