# MVU 模块医生（Kemini Clean 0.10.22 候选）

**尚未完成真实门禁，未锁定，不是已可用版本。** 当前仅启用独立的 MVU 变量修复模块。人物档案与世界引擎按顺序稍后制作，旧业务入口不加载。正文预设、MVU和数据库仍各自运行。

0.10.22 调整分组核对顺序：先引用原卡决定字段范围和写入时机的 check，再判断条件，最后比较状态差额。0.10.21 第四轮把独立背景记录补进尚未满足条件的 MVU 字段，已拒绝。冻结失败材料的两组只读对照正确，但不计验收；完整背景、数据库资料、创建能力、分组和官方执行链均保留。见[字段条件核对来源](docs/modular/PHASE1_CHECK_FIRST.md)。新版本须从全新聊天重新验收。
