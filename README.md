...
当前变量模块为 **0.10.32 候选**。真实正文请求中断时，Doctor正确停止且没有新模型调用，但面板沿用上一回合的请求计数。本次只在新正文生成开始时清零本轮显示计数，避免把旧调用误算到错误回合；实际一次请求上限和失败后不自动重试保持。见[计数修复来源与边界](docs/modular/PHASE1_REQUEST_COUNTER_SOURCE_MAP.md)。当前候选**尚未完成真实门禁**。

0.10.31 已加入前端变量稳定等待：真实测试发现前端在初始化后清空自己管理的日志，连续两轮令变量诊断过期，其中一次已经消耗模型请求；该版本复用旧医生的有界稳定等待，在请求前取得最新快照，保留请求后和写入前的严格保护。0.10.30 的原锁与证据已原样归档，不作为新字节的验收。见[稳定等待来源与边界](docs/modular/PHASE1_FRONTEND_SETTLE_SOURCE_MAP.md)。

人物档案 **0.1.0-candidate.11** 的单次发现、完整44字段新建及旧档增量实现保留，实测完整档案仍为0，未锁定。世界适配保留一次推演和已完成结果复用。P2/P3继续绑定旧上游，须待新P1真实验收后逐模块接入。组合设计预算为 **最多3次医生请求、0次自动重试**；当前候选**尚未完成真实门禁**，不能宣称完整功能或30–60秒目标已实现。见[人物候选说明](docs/profiles/README.md)和[世界适配来源](docs/world/P3_SEMANTIC_REUSE_SOURCE_MAP_2026-09-20.md)。

以下为 **2026-09-17 上一批组合**的历史结果：变量 **0.10.30**、人物档案 **0.1.0-candidate.5**、世界 **0.1.0-candidate.5**，正常医生流程最多 **4 次调用**。这些回合只属于原候选指纹。

同一聊天完成 **12 个普通回合 + 1 个压力回合**。最终 13 份档案完整，世界推进到第 13 轮；一条公开世界后果在下一轮正文中被独立观察到。三类修复按钮、持久化和前端均有真实核验记录；一次刷新后数据恢复，新增调用 0。

按用户允许保留模型例外的范围完成本轮功能核验。模型内容误判、未恢复档案候选、公开世界因果归因风险及 UI 文本键序差异保留，**严格零错误口径仍为“尚未完成真实门禁”，未正式发布。** 本批整座酒馆 107 次调用，其中医生 69 次，报告总量 6,851,274 tokens；已包含 7 次手动修复及联动。版本、安装、费用和限制见 [当前交付说明](docs/MODULAR_CURRENT_STATUS_2026-09-17.md)。

以下为历史阶段记录，不代表当前版本或当前验收结论；旧锁和旧证据继续保留。

**MVU 变量模块已完成阶段验收并锁定；完整医生尚未完成真实门禁。** 同一聊天完成十二个正常回合及一个额外压力回合，实际修复按钮、存档与完成态刷新后恢复均有独立记录。模型仍可能漏判，详情见[阶段验收与使用说明](docs/modular/PHASE1_ACCEPTANCE_0.10.26.md)。

人物档案模块 **0.1.0-candidate.2 已完成阶段验收并锁定**：医生根据普通正文、MVU和权威设定自己生成完整档案并填表，无需预设专用标签。新聊天完成十二个正常回合、一个多人压力回合和一次刷新恢复，最终 30 份记录均为 44 个字段；实际修复按钮与持久化读回已核验，模型内容限制仍保留。安装包为 [profiles/install.json](profiles/install.json)，详见[当前锁定与使用记录](docs/profiles/PHASE2_LOCK_RECORD.md)。接下来适配世界引擎并验证三模块组合；完整医生尚未完成真实门禁。

0.10.26 修复关闭自动检查后手动按钮失效、正文完成后按钮仍灰色，以及变量重读丢失手动模式的问题。复用旧医生的自动/手动独立控制，仅适配当前模块入口与重读参数；模型提示词不变。模型误判先使用“重试本轮”重新检查并修复，只有实际代码缺陷才改代码。见[手动修复来源与验证边界](docs/modular/PHASE1_MANUAL_REPAIR.md)。

0.10.25 每组先复用故事神谕的只读问答核对字段事实，再由原诊断流程完整检查并生成补丁。0.10.24 第四轮因开场时间覆盖已发生流逝拒绝，前三轮不跨候选累计。冻结错误组撤回误修，另一组仍保留必要关系补写，均未执行、不计验收。完整背景、规则、历史、初始化权限、原分组与官方原子保存保留。详见[修复事实核对来源](docs/modular/PHASE1_OBSERVED_REPAIR.md)。尚未完成真实门禁，需新聊天从零验收。

0.10.24 修复同一规则的伴随字段漏核对：状态变化时，同步完成原 check 规定的配套内容。0.10.23 正常第七轮因状态已改变而关联字段仍留占位值拒绝；此前六轮与压力轮不跨候选累计。冻结真实请求的一次只读诊断完成了配套定义，未执行，不计验收。完整背景、历史、创建与初始化权限、分组和官方执行链保留，未采用先前无效的数据库说明草稿。见[状态变化完整性来源](docs/modular/PHASE1_TRANSITION_COMPLETENESS.md)。尚未完成真实门禁，须新聊天重新验收。

0.10.23 限定现有创建登记说明的适用范围：按原卡规则照实填写用户给定的创建值，同时要求其它字段的新增定义满足各自条件。0.10.22 第四轮把既有能力的使用改写成新取得的数值技能，错误已经存档，整轮拒绝。冻结初次和自动重试请求的两次只读对照正确，但不计验收。完整背景、显式创建、规则授权的初始化、原模型与官方执行链保留，未加入第二层语义审查。见[创建登记范围来源](docs/modular/PHASE1_CREATION_SCOPE.md)。新候选须从全新聊天重新完成十二轮。

0.10.22 调整分组核对顺序：先引用原卡决定字段范围和写入时机的 check，再判断条件，最后比较状态差额。见[字段条件核对来源](docs/modular/PHASE1_CHECK_FIRST.md)。

0.10.20 已在正常第5轮拒绝：NPC称号写成玩家结构，官方解析丢弃子字段后仍被医生误报成功。0.10.21 读取当前启用的原卡MVU结构声明；对独立对象插入/替换中实际丢失的子键，使用原有分组重试，只在修复后统一保存。合法默认补全和标量归一化仍交给官方MVU，未重新引入本地补丁执行器。见[来源与边界](docs/modular/PHASE1_EMBEDDED_SCHEMA.md)。尚须全新聊天真实验收，人物和世界模块未开始。

0.10.20 将复杂对象和数组的完整范围各自独立核对，简单字段仍最多八项一组；全部组完成后统一解析、保存。0.10.19 首轮仍漏登记已交互对象，已拒绝。冻结材料只缩小核对范围后补出了遗漏，支持此次分组候选；更多请求会增加耗时，仍须完整真实验收。见[容器独立核对](docs/modular/PHASE1_CONTAINER_GROUPS.md)。

0.10.19 只在医生读取的背景中适配数据库固定的记录说明，保留全部表值和历史，避免正文生成用的绝对优先权覆盖 MVU 字段条件。0.10.18 第四轮把未满足条件的背景任务写进当前副本任务，已拒绝；冻结材料只读定位正确也不能代替新聊天十二轮。见[数据库来源说明适配](docs/modular/PHASE1_DATABASE_WRAPPER.md)。

0.10.18 恢复完整材料在前、唯一核对任务在后的顺序，分组路径和字段原文通过既有消息构造器传入。见[分组任务顺序](docs/modular/PHASE1_GROUP_TASK_ORDER.md)。

0.10.17 为每组变量就近提供完整字段规则原文与写前值，最小适配数据库按目标表构造填表材料的成熟机制。背景、历史和初始化能力完整保留，官方执行与保存链不变。0.10.16 第三轮把数据库背景任务错写到尚未触发的当前副本任务，已拒绝；只读定位正确不计验收，新候选仍需从全新聊天完成十二轮。见[分组原规则来源](docs/modular/PHASE1_GROUP_RULE_MATERIAL.md)。

0.10.15 修正关系字段的触发解释：按本卡规则和实际交互登记，不以“系统投影”或额外的“实质关系”门槛漏掉已交互对象；纯机制通知仍不虚构成人物，登记也不自动增加好感。最小适配 Story Oracle 对系统实体具体交互的处理，仅替换现有变量任务的一段说明，见[交互触发来源](docs/modular/PHASE1_INTERACTION_TRIGGER.md)。0.10.14 搭配本地 0.10.14r1 的第二次真实聊天首轮已拒绝；新候选仍搭配该完整预设，从零验收。
...
0.10.10真实首轮仍漏修提前装备和已发生互动，错误已保存，整轮已拒绝、0/12。0.10.11修复独立发现的边界缺陷：规则明确交给前端的字段，即使当前缺失，医生也不能越权写入；保留既有的精确路径与主语匹配，不封锁其他人物的同名字段。见[缺失字段保护来源](docs/modular/PHASE1_MISSING_OWNERSHIP.md)。这一修复不能宣称已解决此前的模型语义误判；提示词、分组和官方MVU事务未改变，仍须当前候选全新聊天十二轮真实验收后才能锁定。

0.10.12修复真实重生成中医生丢失任务的事件边界：宿主先发生成开始，再删除旧回复；医生现在保留同一作用域、准确楼层的一次预期内部删除，继续等待新正文完成后检查。其余删除仍取消旧任务。复用原有最终正文绑定与存档链，只适配删除监听，不修改预设、模型或数据库。见[重生成来源与边界](docs/modular/PHASE1_REGENERATE_LIFECYCLE.md)。当前尚未真实验收，旧轮次不能累计。

0.10.13修复“部分命令未执行却整体报告成功”：读取官方Zod处理后的未执行命令收据，全部修复完成后才保存。完整保留官方归一化结果，未引入本地补丁执行器。字段核对补充原规则中与check同级的type、format和允许值，避免把占位内容当成符合格式。0.10.12真实同聊前三轮通过、第四轮拒绝；新候选须从0/12开始。来源及实际克隆诊断见[官方执行收据](docs/modular/PHASE1_PATCH_OUTCOME.md)。本地配套0.10.13r1预设只补强现有拍板卡的行动边界一致性，旧作者基线完整保留；尚未完成真实门禁。

0.10.13 的真实第三轮再次拒绝：历史玩家消息被数据库全局正则删空，正文改变已确认物品类型，Doctor 随后按错误类型补全库存。0.10.14 保留 Story Oracle 历史顺序、深度与助手正则，参考 LifeState 分流用户输入，并按数据库原包装提取玩家原话。配套[数据库兼容正则](compatibility/database-user-history.regex.json)保留较早的玩家输入，只裁掉旧召回尾段；本地预设仍为完整的 0.10.13r1。真实现场只读投影已确认原设定恢复，但未调用模型，不能算功能验收。来源见[历史输入修复](docs/modular/PHASE1_USER_HISTORY.md)。必须安装此完整组合，从全新聊天开始，尚未完成真实门禁。

以下为旧版历史，旧入口源码保留供核对，原 manifest/package 已保存到 `legacy/0.9.11/`。

## 0.9.11 历史

**当前状态：0.9.11 首轮真实核验失败，十二回合通过数为 0。请勿将本仓当前版本当作已可用版本。** 变量仍有误修和漏检；世界记录落库但没有产生新的自主行动。人物本轮完整生成一张，桌面面板能读回展示；这不抵消变量与世界失败。

0.9.11 删除重复的五栏诊断提示，直接使用故事神谕原生变量规则和整态审计；保留固定楼层、官方 MVU 执行与读回。原生手选世界书仍优先，无变量机制条目的卡仍回退原生扫描。上一版真实首轮发现的变量、人物和世界问题尚未全部解决，本版本不能当作可用结论。仍配对本地 0.9.9r2 预设，尚未完成真实十二回合。来源与只读对照见 [0.9.11 说明](docs/0.9.11-NATIVE-DIAGNOSIS-CONTEXT.md)。

历史记录：0.9.9 曾尝试用五栏证据修复变量误判，但后续真实测试仍失败，该诊断提示现已被 0.9.11 替代。其固定楼层、官方 MVU 单次解析、非空零效果警告和 World 原生蓝绿灯选择仍保留。上述机制的存在不代表人物与世界已经通过功能验收。
...
人物性格随机、多样性、篇幅、文风、视角和玩家边界仍由配对的 Izumi 预设在正文生成前负责，Doctor 不在正文后重新掷人格。用户的私有预设没有进入本仓库。0.9.9 当前配对候选是 `01_主预设_人物万花筒_IZUMI0825完整保留_MVU适配_0.9.9r2玩家主观边界根修候选版.json`（SHA-256 `DDFC555352C8D1B25EA389D37DE7AA23ADAAF5FF19B9D860D952C9140EEEFD8B`），由 `01_主预设_人物万花筒_IZUMI0825完整保留_MVU适配_0.9.5r2原生World与变量路径根修候选版.json`（SHA-256 `A7ACC663A386830164D3DFACD4C76BAC57019D6072C40FE79EBB626C918EF446`）确定性生成；0.9.9r1 已被 r2 取代。生成只改既有四个适配条目 `user_lite_second_person_0825_v1`、`user_lite_output_0825_v1`、`user_lite_thought_0825_v1`、`c49b9c31-1302-42da-908c-746d39565779`，247 个 prompts、原顺序和启用状态不变。`user_lite_thought_0825_v1` 对糖糖公司防抢话边界做最小适配：禁止新增玩家主动行动、对白、思想和主观反应，同时保留 Master 已明确 A 的必要自然微动作与外力／正式裁决的客观后果；客观接触、位移、伤害或姿态不得反推玩家感受、情绪、同意或反应。

## 世界后台与防全知

World Engine 的持久状态和原生推演始终保留完整数据，并继续接收原件自己的世界状态、相关世界书、原生 Memory Engine 有界上下文、最近对话和原生 `tonePrompt`。0.9.5 起不再改写 Memory 上下文；人物任务只在原生随机任务之后、输出合同之后的 `extraInstruction` 位置出现。它最多携带三名非玩家人物、总段不超过约 2,200 字符，第一名按未终局事件或 World round 选为优先候选；若该候选本轮只能等待玩家，则直接回退到原生 World 已知的既存主体。它不发送玩家、原始 MVU、证据链、人物历史或整份人物库。NPC 已实施且无目击无痕、又需要跨轮保护知情边界的行动沿用原生 `blackbox.secretActions`；已出现具体迹象的筹划、试探、调查起步和矛盾初现沿用 World 3.0.2 的 `events` 早期创建规则。完整 World 供后续推演，公开投影移除 `blackbox` 并按原生事件／风声等级筛选，阻止正文提前全知。最近对话进入原生世界提示前，World 自己的 `evolveFilterRegex` 仍完整清理闭合和被截断的 Story Oracle `<UpdateVariable>` 机制块。
...
`WORLD_ENGINE_EVOLUTION.evolve` 入口另保留直接 API 防线：在等待同楼流水线和调用原函数前，只清理本次 `aiMsg` 与 `opts.dialogueText` 的内存副本，覆盖绕开 Core 对话过滤器的直接调用；它不是第二个调度器。只有原始 `aiMsg` 确有助手正文时才等待同 generationKey 的人物流水线终态；原始 `aiMsg` 为空说明原生手动路径没有可绑定的助手楼，直接交还原件。判断使用过滤前的原文，因此“只有 MVU 机制块的助手回复”仍会进入身份门。失败、取消和无法绑定都按可恢复语义释放，不增加业务超时；默认公开 `waitForWorldDiagnosis` 仍保留“诊断结束即释放”的兼容行为，只有 World 接缝显式请求 `throughProfile`。并发手动复检会建立同 generationKey 的 `manualDiagnosisBinding` 与单调 token，只有仍持有当前 token 的复检才能在 `finally` 清理绑定，旧复检结束不能误删后来开始的新绑定。
