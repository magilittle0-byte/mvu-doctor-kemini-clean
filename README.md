# MVU 人物与世界医生（Kemini Clean 0.9.10）

0.9.10 复用已有运输错误识别，修正宿主将 `[API 错误]` 作为助手楼保存后，Doctor 仍补写变量、人物并触发世界推演的问题。只改现有输入适配入口，Story Oracle 与 World 原件不变；真正正文缺少变量块时仍可正常补全。仍配对本地 0.9.9r2 预设，尚未完成真实十二回合。来源与实际失败见 [0.9.10 说明](docs/0.9.10-HOST-ERROR-REPLY-ADAPTER.md)。

0.9.9 修正 0.9.8 真实新聊天首轮的两项假成功。变量医生恢复成熟旧 Doctor 的 `pre → post → 原更新 → 触发输入 → 最终正文` 五栏证据，并继续只调用一次 Story Oracle、只让官方 MVU 解析一次；“尚可领取/尝试”不再默认等于“已经持有/完成”，无精确规则时也不得擅自改变容器类型。非空修复若被官方 MVU 解析成零变化，会显示可手动复检的警告，而不是绿色正确。世界侧直接复用 World 3.0.2 自带的蓝绿灯激活：只有用户从未保存过 `worldbookTrigger` 时才默认开启，显式开关完全保留；全选仍只是候选池，不再把整本大世界书无条件塞入每轮推演。没有新增第二变量执行器、第二世界引擎或自建世界书检索器。
