...
当前安装：变量 **0.10.30**、人物档案 **0.1.0-candidate.5**、世界 **0.1.0-candidate.4**。三个模块的实现和修复机制已冻结，正常医生流程最多 **4次调用**，无自动重试。医生依据正文、MVU和设定自行设计完整44字段人物档案并填表，无需预设专用档案输出。

同一聊天12个普通回合加1个压力回合共91次调用（医生53，其余38），报告总量5,461,994 tokens。最终5份档案完整，世界状态持续保存；一次刷新后数据恢复，新增调用0。世界离场事件被正文采用尚未证实，模型内容错误和世界UI文本顺序差异保留。**尚未完成完整真实门禁。** 版本、安装、费用和限制以[当前交付说明](docs/MODULAR_CURRENT_STATUS_2026-09-17.md)为准。
