# MVU 人物与世界医生（Kemini Clean 0.9.8）

0.9.8 修正 0.9.7 真实新聊天首轮发现的共享 API 兼容缺口：代理把 `failed to read request body` 之类的运输错误包装进 HTTP 成功响应的 `choices[].message.content`，原生 World 因而把它当模型正文交给 JSON 解析器并正确回滚，但默认零重试使世界轮次停在 0。现在只在既有共享 API 串行适配层复用 Story Oracle 已验证的窄错误信封识别；首次命中以完全相同参数立即重试一次，第二次仍是明确运输错误才抛出无原文、无凭据的类型化失败。识别只接受位于整条返回开头的错误信封或短状态回执，合法 JSON 内部提及 HTTP 错误不会触发。串行层收据也增加版本和已安装函数，0.9.7 同页升级不会继续挂着旧包装器。World 3.0.2 的提示、JSON 解析、合并、骰子、回滚、保存、轮次和注入均未改变。
