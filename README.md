# MVU 模块医生（Kemini Clean 0.10.26 · 变量模块已锁定）
...
人物档案现有独立测试候选 **0.1.0-candidate.1**：医生根据普通正文、MVU和权威设定自己生成完整档案并填表，支持修复按钮、分支存档和后续相关人物召回。安装包为 [profiles/install.json](profiles/install.json)，见[人物模块说明](docs/profiles/README.md)。人物模块尚未完成真实验收或锁定，世界引擎尚未制作；旧业务入口不加载。正文预设、MVU和数据库仍各自运行。
