# MVU 模块医生（Kemini Clean 0.10.18 候选）

**尚未完成真实门禁，未锁定，不是已可用版本。** 当前仅启用独立的 MVU 变量修复模块。人物档案与世界引擎按顺序稍后制作，旧业务入口不加载。正文预设、MVU和数据库仍各自运行。

0.10.18 恢复完整材料在前、唯一核对任务在后的顺序，分组路径和字段原文通过既有消息构造器传入。没有增加提示规则或模型；0.10.17 首轮关系漏记已拒绝，只读定位不能替代新聊天十二轮。见[分组任务顺序](docs/modular/PHASE1_GROUP_TASK_ORDER.md)。
