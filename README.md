...
当前正在实施性能候选：变量 **0.10.30** 原字节保留，人物档案 **0.1.0-candidate.9** 一次请求发现、新建完整44字段、增量维护旧档案。candidate.8 的真实首轮及一次人物按钮修复均因人物证据来源错误被拒绝，未保存完整档案。candidate.9 补齐整体校验失败的固定人工修复诊断，并恢复旧版关于正常字段完整填写的要求；解析规则、模型请求上限和存储机制不变，不能据此宣称模型错误已消除。世界适配保留一次推演，并核对相同事实时能否复用已完成结果。组合正常预算为 **最多3次医生请求、0次自动重试**；当前候选**尚未完成真实门禁**，费用和延迟目标尚未证实。先完成新人物模块真实阶段锁，再接回世界模块完成全链验收。见[人物候选说明](docs/profiles/README.md)和[世界适配来源](docs/world/P3_SEMANTIC_REUSE_SOURCE_MAP_2026-09-20.md)。
