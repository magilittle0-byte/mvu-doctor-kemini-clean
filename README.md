...
当前正在实施性能候选：变量 **0.10.30** 原字节保留，人物档案 **0.1.0-candidate.11** 一次请求发现、新建完整44字段、增量维护旧档案。candidate.10 在三个普通发送和一次自然重生成后完整档案仍为0，实际按钮修复也未恢复，已封存为未通过。candidate.11 仅明确两层 evidence 的不同用途，以及非人个体不适用字段和列表完整性要求；正文证据、身份、44字段校验及调用预算不变，不保证模型内容错误已消除。世界适配保留一次推演，并核对相同事实时能否复用已完成结果。组合正常预算为 **最多3次医生请求、0次自动重试**；当前候选**尚未完成真实门禁**，费用和延迟目标尚未证实。先完成人物模块真实阶段锁，再接回世界模块完成全链验收。见[人物候选说明](docs/profiles/README.md)和[世界适配来源](docs/world/P3_SEMANTIC_REUSE_SOURCE_MAP_2026-09-20.md)。
