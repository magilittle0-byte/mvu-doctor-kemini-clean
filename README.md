...
当前正在实施性能候选：变量 **0.10.30** 原字节保留，人物档案 **0.1.0-candidate.8** 一次请求发现、新建完整44字段、增量维护旧档案，并分开档案任务与背景资料。candidate.7 的真实测试中，两个正常回合的 P2 自动与手动请求均被严格解析拒绝；candidate.8 只调整已绑定整体格式失败的人工反馈和提示顺序，尚无证据确定失败原因。严格解析保留，必须重新完成真实验收。世界适配保留一次推演，并核对相同事实时能否复用已完成结果。组合正常预算为 **最多3次医生请求、0次自动重试**；当前候选**尚未完成真实门禁**，实际耗时仍待测量。先完成新人物模块真实阶段锁，再接回世界模块完成全链验收。见[人物候选说明](docs/profiles/README.md)和[世界适配来源](docs/world/P3_SEMANTIC_REUSE_SOURCE_MAP_2026-09-20.md)。
