...
本文件记录锁定记录的读取边界。当前 0.10.33 已按用户明确的运行链与修复机制标准完成同一聊天十二个有效普通回合及额外压力回合核验，完成一次刷新后的原存档读回；当前阶段结果和模型例外见 `../0.10.33-P1-ACCEPTANCE.md`。严格语义与完整三模块验收仍未通过。0.10.30 等旧版本记录只作历史资料。
...
锁摘要只记录独立真实阶段验收结论，不会因模型自报成功或快速检查通过自动生成。模型候选失败但被正确隔离的回合必须明确保存 `failed` 与 `repairRecovered:false`；不得把机制核验写成语义修复成功，也不得把 P1 阶段锁写成三模块全部完成。
