...
- `actorActions` 是人物的准备或尝试，不是成功事实；必须说明预计耗时与风险。
- 每个 `adjudication` 必须说明实际耗时和私有裁决结果。`success/partial` 只有在至少一条完整 `appliedStateChanges` 已经真实落地时才能提交；只有摘要、没有状态变化的返回会在事务准备前失败关闭。
- `visibility=observable` 的成功或部分成功必须提供不泄露隐秘原因的 `observableConsequence`。
- `hidden/rumor` 行动允许当前没有公开表象，以保护镜头外行动和隐藏动机；但必须有真实私密状态变化以及非空 `revealPath`。这些字段仍属于 Doctor 私有层，不会自动获得正文知情权。
- 格式缺省继续由本地归一化承担；本门只拒绝缺少时间、风险、落地变化或揭示边界的语义空壳，不增加第二套世界状态机。
